...
devsecops-trading-platform-lab/
  ...
  tests/
    utils/
      __init__.py
      base.py               # 공통 유틸 (check, save_history, print_summary)
    smoke/
      test_api_smoke.py  # 전체 API 기본 동작 확인
    validation/
      test_validation.py   # 검증 규칙 전용
    integration/
      test_integration.py  # 계좌→주문 연동 흐름
    security/
      test_security.py     # 입력값 조작/보안 시나리오
    e2e/
      test_e2e.py          # 전체 거래 흐름
    README.md
...
[
  {
    ...
    "created_at": "2026-06-16T04:45:00.123456"
  }
]
```
...
[
  {
    ...
    "price": 190.5,
    "status": "NEW",
    "created_at": "2026-06-16T05:00:00.123456"
  }
]
```
...
python tests/validation/test_validation.py
...
| 파일 | 테스트 수 | 상태 | 설명 |
|------|-----------|------|------|
| `smoke/test_api_smoke.py` | 19개 | ✅ 유효 | 전체 API 기본 동작 확인 |
| `validation/test_validation.py` | - | ⚠️ 일부 유효 | DB 연동 후 완성 예정 |
| `integration/test_integration.py` | - | ⚠️ 일부 유효 | DB 연동 후 완성 예정 |
| `security/test_security.py` | - | ⚠️ 일부 유효 | 인증 추가 후 완성 예정 |
| `e2e/test_e2e.py` | - | ⚠️ 일부 유효 | 도메인 확장 후 완성 예정 |
...
- 도메인 검증 규칙 (quantity, price, account_id 등)
